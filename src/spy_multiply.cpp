...
Spy operator*(const Spy& a, const Spy& b) {
	...
	if (a.dim == b.dim) { // Equal dimensions
		out = a.is_scalar() ?
			...
			a.tape.rec< MultiplyVectorVector< true, true > >({ a, b });
	}
	else { // Unequal dimensions
		if (a.is_scalar()) {
			/* Commutative op */
			out = a.tape.rec< MultiplyVectorScalar< true, true > >({ b, a });
		}
		else if (b.is_scalar()) {
			out = a.tape.rec< MultiplyVectorScalar< true, true > >({ a, b });
		}
		else {
			assert(false);
			throw std::logic_error("Uncompatible tensor dimensions");
...
Spy operator*(const Spy& a, const Tensor& b) {
	...
	if (a.dim == b.dim) { // Equal dimensions
		out = a.is_scalar() ?
			...
			a.tape.rec< MultiplyVectorVector< true, false > >(a, b);
	}
	else { // Unequal dimensions
		if (a.is_scalar()) {
			/* Commutative op */
			out = a.tape.rec< MultiplyVectorScalar< false, true > >(a, b);
		}
		else if (b.is_scalar()) {
			out = a.tape.rec< MultiplyVectorScalar< true, false > >(a, b);
		}
		else {
			assert(false);
			throw std::logic_error("Uncompatible tensor dimensions");
